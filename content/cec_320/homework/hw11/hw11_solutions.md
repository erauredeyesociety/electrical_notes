...
- **Then branch** — `return (int32_t)*(++ptrA) * 2;`
  - Prefix `++ptrA`: advance `ptrA` by `sizeof(int16_t) = 2` bytes *before* loading → pre-indexed `ldrsh r0, [r2, #2]!`.
  - `(int32_t)*ptrA`: `LDRSH` sign-extends the halfword to 32 bits automatically.
  - `* 2`: `lsl r0, r0, #1`.
- **Else branch** — `return *(ptrB++) / 2;`
  - Postfix `ptrB++`: load from the *old* `ptrB`, then advance by `sizeof(int32_t) = 4` → post-indexed `ldr r0, [r3], #4`.
  - `/ 2` (signed): `asr r0, r0, #1`.

---

### Part 1 (20 pts) — Positive Logic

Positive logic branches to `then` when the C test (`x > y`, signed) is true, so the `else` block is laid out first.

```asm
    .global x_cmp_y_load_pl
    .text
...
- `T` slots (1 and 2) run when `le` is true → they implement the C `else` branch.
- `E` slots (3 and 4) run when `gt` is true (inverse of `le`) → they implement the C `then` branch.
...
**Trace check (signed comparisons):**

| `x` | Path | Returned value |
|-----|------|----------------|
| −6 | `bge` not taken → `then`  | −10 |
| −5 | `bge` taken → `elseif`, `blt` taken → `else` | 2·(−5) = −10 |
| 0  | `bge` taken → `elseif`, `blt` taken → `else` | 0 |
| 5  | `bge` taken → `elseif`, `blt` not taken | 10 |
| 7  | `bge` taken → `elseif`, `blt` not taken | 10 |
...
| `x` | First IT | Second IT | `r1` | Result |
|-----|----------|-----------|------|--------|
| −6  | `lt` true → `r1=-10`, `blt` taken | skipped | −10 | −10 |
| −5  | `lt` false → fall through | `ge` false → `lsllt r1, r0, #1` | −10 | −10 |
| 0   | fall through | `ge` false → `r1 = 0` | 0 | 0 |
| 4   | fall through | `ge` false → `r1 = 8` | 8 | 8 |
| 5   | fall through | `ge` true → `r1 = 10` | 10 | 10 |
| 7   | fall through | `ge` true → `r1 = 10` | 10 | 10 |
...
| `x` | 1st test (`ble`) | 2nd test (`bge`) | Branch | Result |
|-----|------------------|------------------|--------|--------|
| 15  | taken | — | then | `*++ptrA * 4` |
| 20  | taken | — | then | `*++ptrA * 4` |
| 21  | not taken | not taken | else | `*++ptrB / 4` |
| 24  | not taken | not taken | else | `*++ptrB / 4` |
| 25  | not taken | taken | then | `*++ptrA * 4` |
| 30  | not taken | taken | then | `*++ptrA * 4` |

---

### Part 2 (20 pts) — CEX Approach (as much as possible)
...
**Why `ITTEE ge` here:**
- The second sub-test is `x >= 25` → CCS `ge` is the "condition is true" case (take the C `then` branch).
- `T` slots run when `ge` is true → the two then-branch instructions.
- `E` slots run when `lt` is true (inverse of `ge`) → the two else-branch instructions.
- Pre-indexed conditional `LDRH`/`LDR` are supported (Lctr 24, Example 4 shows `ldrhhi r0, [r2, #4]!`).

**Trace check:**

| `x` | First `bgt` | Second `cmp` | `ittee ge` branch | Result |
|-----|-------------|--------------|-------------------|--------|
| 15  | not taken (15 ≤ 20) | — | — | `*++ptrA * 4` (fall-through then) |
| 20  | not taken (20 ≤ 20) | — | — | `*++ptrA * 4` |
| 21  | taken → check25 | 21 < 25 → `lt` | E slots | `*++ptrB / 4` |
| 24  | taken → check25 | 24 < 25 → `lt` | E slots | `*++ptrB / 4` |
| 25  | taken → check25 | 25 == 25 → `ge` | T slots | `*++ptrA * 4` |
| 30  | taken → check25 | 30 > 25 → `ge` | T slots | `*++ptrA * 4` |

All rows match the C semantics.
