...
\toprule
flags N=0,Z=0,C=1,V=0 & GT & GE & LE & LT & HI & HS & LS & LO \\
\midrule
eval (both cases)     & T  & T  & F  & F  & T  & T  & F  & F  \\
\bottomrule
\end{tabular}
\end{center}
\begin{hwp}
\textbf{P2 takeaway:} CCS evaluation is \emph{flag-driven, not value-driven}. Both case (a) and (b) yield identical NZCV $(0,0,1,0)$ so all 8 CCSs evaluate identically: GT/GE true (V$=$N \& Z$=$0); LT/LE false; HI/HS true (C$=$1 \& Z$=$0); LS/LO false. Same pattern supports ``$22{>}20$'' signed \emph{and} ``$44{>}_u 42$'' unsigned. \textbf{P2 Part 3:} \cee{EQ <==> Z==1}, \cee{NE <==> Z==0} --- domain-independent.
\end{hwp}
...
(d) 0xH3H2H1H0 -> 0xH0_~H3_H2_H1   (~ = 1's-comp digit)
    ...
    A = ((A >> 4) & 0x00FF)   // H2,H1 -> slots 1,0; H3 falls off
      | (h3c << 8)            // complemented H3 -> slot 2
      | (h0  << 12);          // H0 -> slot 3 (top)

(e) 0xH3H2H1H0 -> 0xF_H0_H3_H2
    uint16_t h0 = A & 0xF;
    A = (A >> 8)        // H3 -> slot 1, H2 -> slot 0
      | (h0  << 8)      // H0 -> slot 2
      | (0xFU << 12);   // top = F
    // (A>>8) puts H3 in slot 1, H2 in slot 0; H0 at slot 2.
\end{lstlisting}
