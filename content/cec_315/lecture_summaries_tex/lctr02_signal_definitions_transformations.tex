...
\title{Lecture 2 — Signal Definitions and Transformations}
\author{}
\date{}
\begin{document}
\maketitle
...
\textbf{Source PDF:} \texttt{all\_lectures/cec315-lctr02-sig-def-tran.pdf}
\textbf{Exam coverage:} Exam 1

\textbf{Focus:} Sections 1.1 and 1.2 (Pages 1 to 13)

\section*{2.1 Definition of a Signal}
...
\textit{[Figure: Block diagram — $x(t)$ enters an "Ideal C to D converter" block (with $T_S = 1/f_S$ as a side input) and outputs $x[n] = x(nT_S)$.]}

\textit{[Figure: Side-by-side plots of $x(t)$ (continuous waveform, amplitudes roughly $-0.1$ to $0.2$, $t$ from $-10$ to $10$) and the sampled $x[n]$ at $f_S = 5\,\text{Hz}$ (stem plot, $n$ from $-40$ to $40$).]}

We can reconstruct $x(t)$ from $x[n]$ using a Digital to Analog (D/A) converter (DAC) or Ideal Discrete to Continuous converter.

\textit{[Figure: Block diagram — $x[n]$ enters an "Ideal D to C converter" block (with $T_S = 1/f_S$ as a side input) and outputs $x(t)$.]}

\textit{[Figure: Side-by-side plots showing $x[n]$ at $f_S = 5\,\text{Hz}$ (stem plot) being reconstructed into $x(t)$ (continuous waveform).]}

Mathematical conditions and implementations will be discussed later.

\section*{2.3 Signal Energy and Power}

Terminology is borrowed from physical systems (like resistors or automobile motion).

\subsection*{2.3.1 DC Power Consumed on a Resistor}

\subsection*{2.3.2 DC Power Consumed on a Resistor}

\subsection*{2.3.3 Total Energy}

\begin{itemize}
  \item Expressed as $E_\infty$
  \item Obtained as the integral of $|x(t)|^2$ for CT signals or the sum of $|x[n]|^2$ for DT signals over an infinite interval.
\end{itemize}

\subsection*{2.3.4 Average Power}

\begin{itemize}
  \item Expressed as $P_\infty$
  \item Obtained as the time-average of the signal energy over an infinite interval.
\end{itemize}

\subsection*{2.3.5 Signal Classification Based on Energy and Power}

Signals are classified into three groups:

\begin{itemize}
  \item finite total energy ($P_\infty = 0$),
  \item finite average power ($E_\infty = \infty$),
  \item neither.
\end{itemize}

\section*{2.4 Transformations of the Independent Variable}

\subsection*{2.4.1 Time Shift}

Shifting a signal $x(t)$ to $x(t - t_0)$ represents a delay (if $t_0 > 0$) or an advance (if $t_0 < 0$).

See Figures 1.8 and 1.9.

\subsection*{2.4.2 Time Reversal}

Reflecting a signal about $t = 0$ or $n = 0$ to produce $x(-t)$ or $x[-n]$.

See Figures 1.10 and 1.11.

\subsection*{2.4.3 Time Scaling}

Linearly stretching or compressing the time axis, such as $x(2t)$ or $x(t/2)$.

See Figure 1.12.

\section*{2.5 Periodic Signals}

A signal is periodic if there is a positive value ($T$ for CT, $N$ for DT) such that $x(t) = x(t + T)$ or $x[n] = x[n + N]$ for all time.

\subsection*{2.5.1 Fundamental Period ($T_0$ or $N_0$):}

The smallest positive value for which the periodicity condition holds.

See Figures 1.14 and 1.15.

\subsection*{2.5.2 Recall Pulse-Width Modulation}

\subsection*{2.5.3 Sinusoidal Signals}

\section*{2.6 Even and Odd Signals}

\subsection*{2.6.1 Even Signals}

Symmetrical about the origin, where $x(t) = x(-t)$.

\subsection*{2.6.2 Odd Signals}

Anti-symmetrical about the origin, where $x(t) = -x(-t)$.

See Figure 1.17.

\subsection*{2.6.3 Decomposing a Signal into a Sum of an Even Part and an Odd Part}
